...
plot(x1); hold on;
plot(x2);
legend('R_a (H || H)', 'R_a (CBF at 0.1 || H)'); % legend('R_a (H ||H)', 'R_a (H ||CBF at 0.05)');
xlabel('iterations');
ylabel('divergence')
...
legend({'Healthy vs Healthy', 'Healthy + $1\%$ diseased  vs Healthy'},'interpreter','latex','Location','northeast') % <-- CHANGE MANUALLY (according to spike-in)!!!
