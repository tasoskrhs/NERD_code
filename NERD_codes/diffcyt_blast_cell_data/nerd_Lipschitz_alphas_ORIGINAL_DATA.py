...
"""
# Version based on "nerd_Lipschitz_16D_trainandpredict_alphas.py" with: different input file names, layers.
...
perc = sys.argv[7]  # distance parameter t
t = sys.argv[8]  # distance parameter t
...
#alpha =  0.01 #0.5 #0.99  #hardcoded, 0.99 <-> DKL, 0.5 <-> Hellinger
#No_alpha = 1
#print('alpha: ', alpha)
#print('a= 0.2, 0.5, 0.9!! ')
alpha = np.array([0.9, 0.95, 1.01]) #np.array(np.linspace(0.1, 1.1, 10))#np.array(np.linspace(-2, -0.1, 20))#np.array(np.linspace(0.1, 5.1, 50))#np.array((np.linspace(0.1, 2, 20)))#((np.linspace(0.11, 2.01, 20)))#  <---------------------
#print('a= 0.1, 0.2,... 0.9!! ')
...
if d==6:                       # <-- diffcyt real data
    layers = [d, 16, 16, 8, 1]#[d, 32, 32, 1]#[d, 32, 32, 16, 1]#[d, 16, 16, 1]
    ...
    layers = [d, 16, 16, 8, 1]#[d, 32, 16, 8, 1]#[d, 16, 16, 8, 4, 1]#[d, 16, 16, 8, 1]
elif d==50:
    layers = [d, 64, 64, 32, 1]
elif d==1:                      #MINE
    layers = [d, 8, 8, 4, 1]
...
for j in range(No_alpha):
    beta = lam*(1-alpha[j]) #lam*(1-alpha) #
    gamma = lam*alpha[j] #lam*alpha
    ...
    gradient_penalty = tf.reduce_mean(tf.math.maximum(tf.zeros([slopes.shape[0]], dtype=tf.float32) ,(slopes-1.0))**2)  #||f(x+DeltaX) -f(x)|| /DeltaX  < 1  (Lipschitz with k=5)
    ...
    #D_solver = tf.train.AdamOptimizer(learning_rate=0.002).minimize(total_loss, var_list=theta_D)
