...
if d==2:
    ...
    layers = [d, 32, 32, 16, 1] #[d, 16, 16, 8, 1]#
elif d==50:
    layers = [d, 32, 32, 16, 1] #[d, 64, 64, 32, 1]
elif d==1:                      #MINE
    layers = [d, 8, 8, 4, 1]  # 1, 8, 1
...
for j in range(No_alpha):
    ...
    #print('slopes size', slopes.shape ) # 2000
    #print('-------------')
    ...
    plt.xlabel('Epochs')
    plt.ylabel('D loss')
    plt.savefig('data/out_gaussian_Lip_plots/cgan_Dloss' + str(j)+ 'w_' + t +'.png', bbox_inches='tight')
    plt.close(fig)
