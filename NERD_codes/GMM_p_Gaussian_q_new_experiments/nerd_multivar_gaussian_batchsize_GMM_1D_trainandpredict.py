...
for j in range(No_alpha):  # range(1): # <-------------------
    ...
    # Plotting
    # -----------------------------------------------------------------------------
    if not os.path.exists('data/out_GMM_BS_plots/'):
        os.makedirs('data/out_GMM_BS_plots/')
    ...
    plt.xlabel('Epochs')
    plt.ylabel('D loss')
    plt.savefig('data/out_GMM_BS_plots/cgan_Dloss' + str(j) + 'w_' + t +'.png', bbox_inches='tight')
    plt.close(fig)
...
# GENERATE DATA (from trained model)  only for one alpha value...
#===================================
eval_pts = np.expand_dims(np.linspace(-10, 10, 100) , 1)  # support of x
#print(eval_pts)
#print('shape[0]=',eval_pts.shape)
predictions_real, predictions_fake = sess.run([D_real, D_fake], feed_dict={X: eval_pts, Y: X_mb})
print('predictions_real shape:',predictions_real.shape[0],predictions_real.shape[1])
#print('predictions_fake shape:',predictions_fake.shape[0],predictions_fake.shape[1])

#minib = [eval_pts, eval_pts]
#print(minib)

#fig = plt.figure(figsize=(5, 5))
#plt.plot(eval_pts, predictions_real,'r',X_mb, predictions_fake,'bo') # plot my TRAINED discriminator, samples from Gaussian distr p (meaning x here)
#plt.show()
